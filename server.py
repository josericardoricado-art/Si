...
DEEPGRAM_SOURCE_LANGUAGE = os.environ.get(
    "DEEPGRAM_SOURCE_LANGUAGE", "pt-BR"
).strip()

PIPER_DATA_DIR = Path(
    os.environ.get("PIPER_DATA_DIR", "/app/voices")
)
AUDIO_DIR = Path(
    os.environ.get("AUDIO_DIR", "/tmp/si_audio")
)
...
# ============================================================
# VOZES PIPER
# Os arquivos sÃ£o baixados diretamente do repositÃ³rio oficial
# de vozes do Piper. NÃ£o usamos piper.download_voices.
# ============================================================
...
def deepl_language(language):
    ...
    return mapping.get(normalize_language(language), "PT-BR")
...
# ============================================================
# DOWNLOAD / CARREGAMENTO DO PIPER
# ============================================================
...
def download_file(url, destination):
    ...
    try:
        ...
        if not temporary.exists() or temporary.stat().st_size < 1000:
            raise RuntimeError("Arquivo baixado estÃ¡ vazio ou incompleto.")
...
def download_piper_voice(model_name):
    ...
    config = PIPER_DATA_DIR / f"{model_name}.onnx.json"

    if model and config.exists() and config.stat().st_size > 100:
        return model

    relative_path = PIPER_VOICE_PATHS.get(model_name)
    if not relative_path:
        raise RuntimeError(
            f"Caminho da voz Piper nÃ£o configurado: {model_name}"
        )

    model_path = PIPER_DATA_DIR / f"{model_name}.onnx"
    config_path = PIPER_DATA_DIR / f"{model_name}.onnx.json"
    ...
    if not model_path.exists() or model_path.stat().st_size < 1000000:
        raise RuntimeError(
            f"Modelo Piper nÃ£o foi baixado corretamente: {model_path}"
        )

    if not config_path.exists() or config_path.stat().st_size < 100:
        raise RuntimeError(
            f"ConfiguraÃ§Ã£o Piper nÃ£o foi baixada corretamente: {config_path}"
        )
...
def get_piper_voice(language):
    ...
    with lock:
        ...
        model_path = download_piper_voice(model_name)
...
def generate_piper_audio(text, language):
    ...
    print(
        f"[PIPER] Gerando voz: {text}",
        flush=True,
    )

    try:
        # O Piper 1.2.0 possui synthesize_wav(),
        ...
        raise RuntimeError(
            f"Erro na geração de áudio Piper: {error}"
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"WAV Piper inválido: {error}"
        )

    print(
        f"[PIPER] Áudio pronto: {filename}",
        flush=True,
    )
...
def translate_with_deepl(text, target_language):
    ...
    if not DEEPL_API_KEY:
        raise RuntimeError(
            "DEEPL_API_KEY nÃ£o configurada no Render."
        )
    ...
    if response.status_code != 200:
        raise RuntimeError(
            f"DeepL HTTP {response.status_code}: {response.text[:500]}"
        )
...
def deepgram_worker(job):
    ...
    try:
        if not DEEPGRAM_API_KEY:
            raise RuntimeError(
                "DEEPGRAM_API_KEY nÃ£o configurada."
            )
...
def create_audio_session(job_id, target_language):
    ...
    job = {
        ...
        "audio_id": None,
        "audio_queue": [],
        "audio_queue_input": queue.Queue(maxsize=200),
        "translation_queue": queue.Queue(maxsize=100),
        "error": None,
        "connected": False,
...
def health():
    return jsonify({
        ...
        "piper": True,
        "piper_voices_loaded": list(piper_voices.keys()),
        "piper_models_directory": str(PIPER_DATA_DIR),
        "openai": False,
        "elevenlabs": False,
        "deepl_voice": False,
    })
...
def test_piper():
    if request.method == "POST":
        ...
        text = data.get(
            "text",
            "OlÃ¡. Este Ã© um teste de voz do SI.",
        )
    ...
    try:
        ...
        print("[TEST PIPER] Erro:", repr(error), flush=True)
        return jsonify({
            "ok": False,
            "provider": "Piper",
            "error": str(error),
...
def audio_start():
    try:
        ...
        print("[AUDIO START] Erro:", repr(error), flush=True)
        return jsonify({
            "ok": False,
            "error": str(error),
        }), 500
...
def audio_chunk():
    ...
    if not encoded:
        return jsonify({
            "ok": False,
            "error": "Ãudio nÃ£o informado.",
        }), 400

    target_language = normalize_language(
        data.get(
            "targetLanguage",
            data.get("targetLang", DEFAULT_TARGET_LANGUAGE),
        )
    )
...
def audio_status(job_id):
    ...
    with job["lock"]:
        ...
        return jsonify({
            ...
            "interimText": job["interim_text"],
            "audioUrl": next_audio["audioUrl"] if next_audio else None,
            "audioId": next_audio["audioId"] if next_audio else None,
            "audioQueueSize": len(job["audio_queue"]),
            "error": job["error"],
            "targetLanguage": job["target_language"],
            "deepgramConnected": job["connected"],
        })
...
def stop_audio(job_id):
    ...
    try:
        job["audio_queue_input"].put_nowait(None)
    except Exception:
        pass

    try:
        job["translation_queue"].put_nowait(None)
    except Exception:
        pass
