...
# modelo.onnx.json
#
# Os dois arquivos são baixados diretamente do
# repositório oficial de vozes Piper.
# ============================================================
...
def download_piper_voice(
    ...
    if (
        ...
        config_path.exists()
        and
        model_path.stat().st_size > 1000000
    ):

        print(
            "[PIPER] Modelo já existe:",
            model_path,
            flush=True
        )
    ...
    if not config_path.exists():
        ...
        raise RuntimeError(
            "Configuração Piper não foi baixada: "
            + str(config_path)
        )
    ...
    if model_path.stat().st_size < 1000000:
        ...
        raise RuntimeError(
            "Arquivo ONNX da voz Piper parece inválido."
        )
...
def generate_piper_audio(
    ...
    if output.stat().st_size < 1000:
        ...
        raise RuntimeError(
            "Arquivo WAV criado pelo Piper está vazio."
        )
