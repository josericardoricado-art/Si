...
def normalize_language(language):
    ...
    aliases = {
        ...
        "es-es": "es", "es_es": "es",
        "spanish": "es", "espanhol": "es", "espanol": "es",
        "fr-fr": "fr", "fr_fr": "fr",
        "french": "fr", "frances": "fr", "francês": "fr",
        "de-de": "de", "de_de": "de",
...
def deepl_language(language):
    return {
        ...
        "de": "DE",
        "it": "IT",
    }.get(normalize_language(language), "PT-BR")
...
def download_piper_voice(model_name):
    ...
    if not model:
        raise RuntimeError(f"Modelo Piper não encontrado: {model_name}")
...
def generate_piper_audio(text, language):
    ...
    print(f"[PIPER] Gerando: {text}", flush=True)
...
def translate_with_deepl(text, target_language):
    ...
    if not DEEPL_API_KEY:
        raise RuntimeError("DEEPL_API_KEY não configurada no Render.")
    ...
    if not translations:
        raise RuntimeError("DeepL não retornou tradução.")
...
def process_transcript(job, transcript):
    ...
    try:
        ...
        print("[SI] Tradução:", translated, flush=True)
        ...
        print("[SI] Áudio Piper pronto:", audio_id, flush=True)
...
def translation_worker(job):
    while not job["stop_event"].is_set():
        ...
        try:
            ...
            print("[TRANSLATION] Erro:", repr(error), flush=True)
...
if __name__ == "__main__":
    print("====================================", flush=True)
    print(" SI — TRADUTOR LIVE", flush=True)
...
path.write_text(server_py, encoding="utf-8")

# Also create a requirements file compatible with the server.
requirements = """Flask
flask-cors
requests
websocket-client
